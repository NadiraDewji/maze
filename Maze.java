...
public class Maze {
	 ...
	 */
	public Maze (char [][] charMazeFromFile ) throws IllegalArgumentException {
	 ...
	 */
	public SquareType getSquareType( SquarePosition sp) {
	 ...
	 */
	public SquarePosition generateStartPosition ( ) { 
		//pick coordinates at random 
		int col = rand.nextInt(width);
		int row = rand.nextInt(height); 
		SquarePosition sp = new SquarePosition(row,col);
		...
		while (!getSquareType(sp).canBeSet()){
			...
			sp = new SquarePosition(row,col); 
		}
		//store and return the start position selected 
		start = new SquarePosition(row, col) ; 
		markSquareToStart(); 
		return sp; 
	}
	 ...
	 */
	public SquarePosition setStartPosition (int col, int row ) { 
		SquarePosition sp = new SquarePosition(row,col);
		//keep trying until valid start position coordinates are found 
		//(we cannot start from the wall, for example) 
	 ...
	 */
	public void setSquareToVisited ( SquarePosition sp ) {
		if ((sp.getRow() >=0 && sp.getRow() < height && sp.getColumn() >= 0 && sp.getColumn() < width )
	 ...
	 */
	public void setSquareToExit ( SquarePosition sp ) {
		if ((sp.getRow() >=0 && sp.getRow() < height && sp.getColumn() >= 0 && sp.getColumn() < width )
	 ...
	 */
	public ArrayList<SquarePosition> getNeighbors ( SquarePosition sp ) {
		ArrayList<SquarePosition> list = new ArrayList<SquarePosition>();
		int row = sp.getRow();
		int column = sp.getColumn(); 
		SquarePosition newSquarePostion = null;
		if (row >=0 && row < height && column >= 0 && column < width ) {
			
			if ( row > 0  ) {	//top
				newSquarePostion = new SquarePosition (row-1, column);
			...
			if ( column < width-1  ) {	//right
				newSquarePostion = new SquarePosition (row, column+1);
				if ( ! getSquareType(newSquarePostion).isWall() )
